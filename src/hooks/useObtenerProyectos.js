...
const useObtenerProyectos = () => {
  ...
  const [ultimoProyecto, setUltimoProyecto] = useState(null);
  ...
  const obtenerMasProyectos = async () => {
    try {
      ...
      if (docsCombinados.length > 0) {
        setUltimoProyecto(docsCombinados[docsCombinados.length - 1]);
        setProyectos((prev) =>
          prev.concat(
  ...
  useEffect(() => {
    ...
    const manejarSnapshot = (snapshot1, snapshot2) => {
      ...
      setProyectos(docsCombinados.map((doc) => ({ ...doc.data(), id: doc.id })));
    };
  ...
  return [proyectos, obtenerMasProyectos, hayMasPorCargar];
};
